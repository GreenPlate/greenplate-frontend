...
async function fetchProfile(){
    const profile_data = await fetch(URL1+"/user-as-authenticated", makeOptions("GET", null, true)).then(handleHttpErrors);
    document.querySelector('#username-input').value = sanitizer(profile_data.username);
    document.querySelector('#email-input').value = sanitizer(profile_data.email);
    document.querySelector('#first-name-input').value = sanitizer(profile_data.firstName);
    document.querySelector('#last-name-input').value = sanitizer(profile_data.lastName);
    document.querySelector('#change_data_user').addEventListener('click', profileModalOpen)
    document.querySelector('#change_user_data_btn').addEventListener('click', changeUserData)
}
...
function changeUserData(){
    ...
    fetch(URL1+"/user-as-authenticated", makeOptions("PATCH", patchUser, true)).then(handleHttpErrors);
    location.reload();
}
async function fetchShoppingList(){
    ...
    const totalPrice = list.offers.reduce((total, offer) => total + offer.newPrice, 0);
    ...
    if(shopping_list_new.length === 0){
        document.querySelector('#new-list-field').innerHTML = sanitizer("Der er ikke noget at vise");
    }
    else{
    ...
    if(shopping_list_old.length === 0){
        document.querySelector('#old-list-field').innerHTML = sanitizer("Der er ikke noget at vise");
    }
    else{
        document.querySelector('#old-list-field').innerHTML = sanitizer(old_list);
    }
    document.querySelector('#new-list-field').addEventListener('click', function (event) {
...
function openShoppingList(listId, shoppingListArray) {
    ...
    if (shoppingListArray && shoppingListArray.length > 0) {
        const selectedIndex = shoppingListArray.findIndex(list => list.id === numericListId);
        ...
        const listBody = storeInfo + selectedList.offers.map(offer => { 
            return `
                <div class="col">
                    <div class="row">
        ...
        document.querySelector('#shoppinglist-modal-body').innerHTML = sanitizer(listBody)
...
async function fetchRecipes(){
    const recipe_data = await fetch(URL3+"/user-as-authenticated", makeOptions("GET", null, true)).then(handleHttpErrors);
    const recipe_list = recipe_data.map((recipe) => {return `
...
`;
    ...
    if(recipe_data.length === 0){
        document.querySelector('#recipe-cards').innerHTML = sanitizer("Der er ikke noget at vise");
    }else{
        document.querySelector('#recipe-cards').innerHTML = sanitizer(recipe_list_with_row);
    }
