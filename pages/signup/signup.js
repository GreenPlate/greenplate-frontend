...
async function signup(){
    ...
    const passwordCheck = document.querySelector("#input-password2").value;

    if(!isSignupValid(firstName, lastName, email, username, password, passwordCheck)){
        return;
    }
...
function isSignupValid(firstName, lastName, email, username, password, passwordCheck){
    if(!firstName || firstName.trim() === ""){
        document.querySelector('#invalid-feedback1').innerHTML = "Indtast venligst fornavn."
        return false;
    }

    if(!lastName || lastName.trim() === ""){
        document.querySelector('#invalid-feedback2').innerHTML = "Indtast venligst efternavn."
        return false;
    }

    if(!username || username.trim() === "" || username.length < 2){
        document.querySelector('#invalid-feedback3').innerHTML = "Indtast venligst brugernavn."
        return false;
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if(!email || email.trim() === "" || !emailRegex.test(email)){
        document.querySelector('#invalid-feedback4').innerHTML = "Indtast venlist en gyldig emailadresse."
        return false;        
    }
    
    if(!password || password.length < 4){
        document.querySelector('#invalid-feedback5').innerHTML = "Kodeord skal være mindst 4 tegn."
        return false;
    }
    
    if(password !== passwordCheck){
        document.querySelector('#invalid-feedback6').innerHTML = "Kodeord er ikke ens."
        return false;
    }
