...
async function fetchZipCount() {
    ...
    const zipCountRows = zipCountArray.map(zipCount => `
        <tr>
...
async function fetchStoreCount(){
    ...
    const storeCountArray = [
        {"store": "Store A", "count": 20},
        {"store": "Store B", "count": 15},
        {"store": "Store C", "count": 5},
        {"store": "Store D", "count": 30},
        {"store": "Store E", "count": 12},
      ];
  
      const storeCountRows = storeCountArray.map(storeCount => `
          <tr>
...
function createStoreChart(storeCountArray){
    ...
    const storeChartCtx = document.getElementById('storeCountChart').getContext('2d');
...
async function fetchProductCount(){
    ...
    const productCountRows = productCountArray.map(productCount => `
        <tr>
    ...
    document.querySelector('#product-count-table-body').innerHTML = productCountRows;
}
